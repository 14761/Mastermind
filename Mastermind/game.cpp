...
game::game(bool isRepeat)
{
    ...
    for (int i = 0; i < codeLength; i++) // Iterate 4 times (length of the code)
    {
        ...
        if (!isRepeat) // check if the game allows to repeat letters
        {
            ...
            tempCode += c; // saves the letter in the code
        }
...
void game::checkGuess()
{
    ...
    bool tempGuess[4];
    std::fill(std::begin(tempGuess), std::end(tempGuess), false);
    for (int i = 0; i < codeLength; i++) // iterator on secret code
    {
        ...
        {
            if (tempGuess[j - (round * codeLength)])
            {
                continue;
            }
...
void game::inputGuess()
{
    ...
    do
    {
        ...
        std::cin >> guessTemp;
        transform(guessTemp.begin(), guessTemp.end(), guessTemp.begin(), ::toupper);
        std::cout << std::endl;
